...
import ReactFlow, { ReactFlowProvider, useNodesState, useEdgesState, addEdge, useReactFlow, Background, Controls } from 'reactflow'
import 'reactflow/dist/style.css'

import './index.css'
import LightModeButton from './Components/LightModeButton.js'
import { NodeBottom, NodeTop, NodeLeft, NodeRight, NodeMain } from './Components/Nodes.js'
  ...
  const onConnectEnd = useCallback(
    (event) => {
      ...
      if (targetIsPane) {
        ...
        setEdges((eds) =>
          eds.concat({ id, source: connectingNodeId.current, target: id, type: 'default', sourceHandle: connectingNodeId.handleId })
        )
      }
    },
    [project]
    ...
    >
      ...
      <div className='editContainer'>
        ...
        <div className='updatenode__controls'>
          <text>Node Name:</text>
          <input value={nodeName} onChange={(evt) => setNodeName(evt.target.value)} style={{ width: '250px' }} />
          <text>Link:</text>
          <input value={link} onChange={(evt) => setLink(evt.target.value)} style={{ width: '250px' }} />
